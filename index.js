...
const VAULT_PUBKEY = new PublicKey(VAULT_ADDRESS);
...
function applyBuySplit(tx, userPubkey, lamportsIn) {
  const keepLamports = Math.floor(lamportsIn * transactionDevFee);
  const remaining = lamportsIn - keepLamports;
  const vaultFee = Math.floor(remaining * transactionDevFee);
  const tradeLamports = remaining - vaultFee;
  ...
  return { keepLamports, vaultFee, tradeLamports };
}
...
async function buyToken({ privateKey, mint, sol, slippageBps = 500 }) {
  ...
  if (!curve.complete) {
    ...
    const { tradeLamports } = applyBuySplit(tx, user.publicKey, lamportsIn);
    ...
    const { tradeLamports } = applyBuySplit(instructionCollector, user.publicKey, lamportsIn);
